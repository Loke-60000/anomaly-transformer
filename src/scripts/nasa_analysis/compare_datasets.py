...
# Try to import our existing data loader for comparison
HAS_OUR_LOADER = False
try:
    import sys

    sys.path.append("src")
    sys.path.append("./src")
    from data.data_loader import AnomalyDataLoader, TimeSeriesNodeDataLoader

    HAS_OUR_LOADER = True
except ImportError as e:
    print(f"Note: Could not import our data loader: {e}")

    # Create a simple placeholder
    class TimeSeriesNodeDataLoader:
        @staticmethod
        def load_from_index(path, unit_id, combine_method):
...
def compare_datasets():
    ...
    if os.path.exists(timeseries_path):
        # Load our data
        try:
            our_data = TimeSeriesNodeDataLoader.load_from_index(
                timeseries_path, unit_id="73", combine_method="concatenate"
            )
            print(f"Our data shape: {our_data.shape}")
            print(f"Data type: {our_data.dtype}")
            print(f"Features per timestep: 1 (univariate)")
            print(f"Value range: [{our_data.min():.3f}, {our_data.max():.3f}]")
...
class ImprovedTransformerAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decode
        output = self.decoder_transformer(decoded_input, encoded)
...
1. Multi-Stage Training:
   Stage 1: Pre-train on NASA dataset (supervised)
   Stage 2: Fine-tune on our data (unsupervised)
   Stage 3: Ensemble with uncertainty estimation

2. Loss Function:
   Total Loss = Reconstruction Loss + λ₁ * KL Divergence + λ₂ * Contrastive Loss
...
if __name__ == "__main__":
    ...
    print(f"This is a significant upgrade opportunity!")

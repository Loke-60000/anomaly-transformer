...
class ModelConfig:
    ...
    def __post_init__(self):
        if self.model_type == "lightweight":
            ...
            raise ValueError(
                f"Unknown model_type: {self.model_type}. Choose 'lightweight' or 'full'."
            )
...
class ConfigurationManager:
    ...
    def print_summary(self) -> None:
        ...
        m = self.config.model
        if m.model_type == "lightweight":
            print(f"  Layers: {m.num_layers}")
        else:
            print(f"  Layers: {m.num_encoder_layers}/{m.num_decoder_layers} (enc/dec)")
